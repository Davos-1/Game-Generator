...
1. Nutzer stellt ein Heft zusammen und klickt «Heft kaufen».
2. Die Website schickt die Heft-Konfiguration an den Worker. Der Worker legt
   bei Payrexx eine Bezahlseite an, merkt sich in KV `{Zahlung → Konfigurations-Hash}`
   und gibt die Adresse der Bezahlseite zurück.
3. Der Nutzer bezahlt (Karte oder TWINT) und kehrt zurück auf
   `/raetselheft?zahlung=<id>&status=ok`.
...
5. Das Token gilt für **genau dieses Heft**: es enthält den Hash der
   Konfiguration. Vor dem Erzeugen des sauberen PDFs prüft der Worker das
   Token gegen die aktuelle Konfiguration.
...
In `wrangler.jsonc` stehen Preis (`PRICE_RAPPEN`, Standard 500 für CHF 5.00),
Währung und die erlaubte Herkunft (`SITE_ORIGIN`).
